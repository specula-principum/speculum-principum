...
If network is blocked, close this issue with label `blocked-network`.
...
1. **Fetch and parse** the updated content:
   ...
   storage = ParseStorage(root=paths.get_evidence_root() / "parsed")
   result = parse_single_target("{url}", storage=storage, is_remote=True, force=True)
   ```
...
- [ ] `SourceEntry.last_content_hash` updated in registry
- [ ] Issue closed with update summary
